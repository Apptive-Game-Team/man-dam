...
무엇이 웃긴 만담을 만드는지는 [`docs/funny.md`](docs/funny.md) 에 정리했다. 그 문서가 원본이고 프롬프트와 채점표는 거기서 나온다. 셋 중 하나를 고칠 일이 생기면 문서를 먼저 고친다.

기준을 코드가 어떻게 재는지는 두 층이다.

| 층 | 무엇을 재나 | 어디서 |
|---|---|---|
| `app/review.py` | 웃길 조건을 갖췄나 (전제·계단·오치·문형) | 그래프 안. 떨어지면 다시 쓰게 한다 |
| `app/verify.py` | 실제로 웃겼나, 대화가 이어졌나 | 그래프 밖. 기록된 공연을 감사한다 |

```bash
uv run python -m app.verify   # 채점표 점수와 재미 점수를 나란히 본다
```
